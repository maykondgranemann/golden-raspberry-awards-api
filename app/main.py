...
app = FastAPI(
    ...
    description="API para processar e analisar vencedores do prêmio Golden Raspberry Awards.",
    version="0.0.1"
)

TESTS_CACHE_FILE = "tests/.last_test_run"

def get_last_test_execution():
    ...
    if os.path.exists(TESTS_CACHE_FILE):
        with open(TESTS_CACHE_FILE, "r") as f:
            return datetime.datetime.fromisoformat(f.read()).astimezone(brasilia_tz).isoformat()
